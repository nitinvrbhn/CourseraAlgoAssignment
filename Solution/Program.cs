    ...
    internal static class Program
    {
        private static void Main(string[] args)
        {
            ...
            try
            {
                ...
                var numberOfTimes = Int32.Parse(fileContent[0]);
                for (int i = 1; i<=numberOfTimes;i++)
                {
                    dataArray.Add(Array.ConvertAll(fileContent[i].Split(' '), Int32.Parse));
                    
                }
                var stops = dataArray.Select(i => new Stop(new City(i[0], i[1])))
                                  .NearestNeighbors()
                                  .ToList();
                ...
                file.LogMessage("Process ended.. " + Convert.ToInt32(startingTour.Cost()));
